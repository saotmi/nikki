    ...
    <div v-for="(value, key, index) in shares" :key="index">
      <div class="message">
        <div class="flex">
          ...
          <img class="icon" src="../assets/icon_020010_256.png"
          @click="fav(key)"
          alt />
          <p class="number">{{value.like.length}}</p>
          <img class="icon" src="../assets/icon_058590_256.png" 
          @click="del(key)"
          alt
          v-if="path && profile" />
          <img class="icon" src="../assets/icon_058700_256.png"
          @click="
          $router.push({
        ...
        this.shares[index].like.forEach((element) => {
          if (element.user_id == this.$store.state.user.id) {
            ...
            .then((response) => {
              ...
              this.$router.go({ 
              path: this.$router.currenRoute.path,
              force: true,
            });
        });
      }
